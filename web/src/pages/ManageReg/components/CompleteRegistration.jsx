...
export default ({ visible, disable, registration }) => {
    ...
    const content = () => {
        if (currentStep === 0) return <RegFrom registration={registration} nextStep={nextStep}></RegFrom>;
        if (currentStep === 1) return <OwnerForm registrationId={registration.id} owner={registration.owner} nextStep={nextStep}></OwnerForm>;
